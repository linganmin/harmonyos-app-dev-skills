...
生产桶通常配置**私有读**（涉及用户隐私数据时必须私有）：

- 后端接口下发的文件 URL 带签名（如 `x-oss-expires=3600`），可直接用 `Image()` 等组件加载，但**有效期短，不可持久缓存**。
- 客户端直传成功后自行拼接的裸 URL（`host/objectKey`）**无读取权限，GET 返回 403**。症状：上传 toast/状态全部正常，但 `Image` 加载新 URL 静默失败、画面停留旧图——肉眼极难定位，需抓 hilog 看 DownloadManager/NETSTACK 的 403。
- **正确做法**：上传后的本地回显一律用本地沙箱文件（`file://` + 缓存路径，上传成功前不要删除本地副本）；裸 URL 或 objectKey 只用于提交给后端，由后端存储并在下发时重新签名。
- 后端要求提交「OSS 相对路径」时提交 objectKey（`dir/fileName`），不要带 host；要求 URL 时提交裸 URL。提交前确认接口文档的字段语义。
